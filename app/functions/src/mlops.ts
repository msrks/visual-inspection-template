...
const trainModels = async () => {
  ...
  await Promise.all(
    (
      ...
      .filter(
        (m) =>
          !m.modelId && !["PIPELINE_STATE_QUEUED", "PIPELINE_STATE_PENDING", "PIPELINE_STATE_RUNNING"].includes(m.state || "")
      )
...
const updateModelState = async () => {
  let pipelines = await listTrainingPipelines();
  pipelines = pipelines.sort((a, b) => b.datetime.getTime() - a.datetime.getTime());
  const uniqueIds = [...new Set(pipelines.map((p) => p.datasetId!))];

  await Promise.all(
    uniqueIds.map(async (id) => {
      const p = pipelines.find((p) => p.datasetId === id)!;
...
export const updateModelsOnCall = func.https.onCall(async () => {
  await trainModels();
  await updateModelState();
});
